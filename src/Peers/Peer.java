...
//-----------File managment methods----------
  ...
  //File details
    ...
    public void mergeFileFragments(String fileName) throws IOException, ClassNotFoundException {
        // Path for this peer's shared_directory
        Path currentDir = Paths.get(System.getProperty("user.dir")).resolve("src");
        String shared_dir_path = currentDir.resolve(this.shared_dir).toString();

        // Directory to store the merged file
        File dir = new File(shared_dir_path);

        // File output stream for the merged file
        String mergedFilePath = shared_dir_path + File.separator + fileName;
        try (FileOutputStream fos = new FileOutputStream(mergedFilePath);
            ...
            // Read each fragment file and write its contents to the merged file
            for (File partFile : fragments) {
